...
useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    JsonConfig('put',`api/membership/update/password/${userId}`,requestData).then(()=>{
      console.log('전송 성공')
    }).catch((error)=>{
  ...
  return (
    <div className="relative bg-main-100">
      ...
      <form action="" onSubmit={handleSubmit} method="post">
        ...
        <div className="flex flex-col mt-5 mx-auto w-9/12">
          <label htmlFor="input-changepassword" className="w-9/12 after:content-['*'] after:text-red-500" >변경할 비밀번호</label>
          <input type="password" id="input-changepassword" className="mt-2 h-10 p-2 placeholder:text-sm" value={changepassword} onChange={(e)=>setChangePassword(e.target.value)} name="password" placeholder="비밀번호를 입력해주세요"/>
        </div>
        <div className="flex flex-col mt-5 mx-auto w-9/12">
