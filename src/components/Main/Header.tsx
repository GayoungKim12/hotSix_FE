    ...
    <div className="fixed w-full z-20 top-0 left-0 shadow bg-main-100">
      <div className="flex flex-row justify-between items-center h-16 px-3 ">
        <img src="/logo.png" className="w-16 cursor-pointer" onClick={moveToHome} />
          ...
          <button className="w-16 h-12 bg-main-400" onClick={(e) => {
              e.stopPropagation();
              setIsLogOut(true);
            }}>로그아웃
        </button>  
      ...
      {isLogOut && <Modal type="confirm" content={"로그 아웃을 하시겠습니까?"} handleModal={() => setIsLogOut(false)} handleDelete={() => logoutUser(navigate)} />}
    </div>
